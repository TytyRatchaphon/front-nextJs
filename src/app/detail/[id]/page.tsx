...
import BuyMultipleEpisodesModal from "@/components/BuyMultipleEpisodesModal";
...
export default function BookDetailPage() {
  ...
  // เรียก API ด้วย React Query - User Wallet
  const { data: userWallet } = useQuery({
    queryKey: ["userWallet"],
    queryFn: fetchUserWallet,
    enabled: isLoggedIn, // เรียกเมื่อ login แล้ว
    staleTime: 2 * 60 * 1000, // Cache 2 นาที
  });
                  ...
                  {isExpanded && (
                    <div className="divide-y divide-gray-50">
                      {group.list.map((episode: any) => (
                        <Link
                          key={episode.ep_id}
                          href={`/read/${bookDetail?.bookID}/${episode.epID}`}
      ...
      <div className="text-center px-2 sm:px-4">
        ...
        <div className="space-y-2">
          ...
          <p className="text-xs sm:text-sm text-gray-600">
            - เนื้อเรื่องดี ตัวละครมีมิติ
          </p>
          <p className="text-xs sm:text-sm text-gray-600">
            - อ่านแล้ววางไม่ลงเลยครับ
          </p>
        </div>
      </div>
    ),
