...
export default function CollectionDetail({ collectionId }: Props) {
  ...
  const apiBooks = detailData?.books || [];
  ...
  const { mutate: toggleLike, isPending: isLiking } = useMutation({
    ...
    onMutate: async () => {
      await queryClient.cancelQueries({ queryKey: ['collectionBooks', collectionId] });
      const previousData = queryClient.getQueryData(['collectionBooks', collectionId]);
      
      queryClient.setQueryData(['collectionBooks', collectionId], (old: any) => {
    ...
    onError: (err, variables, context: any) => {
      queryClient.setQueryData(['collectionBooks', collectionId], context?.previousData);
    },
    onSettled: () => {
      queryClient.invalidateQueries({ queryKey: ['collectionBooks', collectionId] });
    }
              ...
              <div>
                <div className="mb-3 flex flex-wrap items-center gap-2">
                  ...
                  <button
                    onClick={() => toggleLike()}
                    disabled={isLiking}
                    className={`inline-flex h-8 items-center gap-1.5 rounded-full px-3 text-xs font-semibold transition-all border ${
                      collection.is_liked 
                        ? 'border-red-500 bg-red-50 text-red-600' 
                        : 'border-gray-200 bg-white text-gray-600 hover:border-red-200 hover:text-red-500 hover:bg-red-50'
                    }`}
                  >
                    <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill={collection.is_liked ? "currentColor" : "none"} stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
        ...
        <section className="mb-8 rounded-[28px] border border-gray-100 bg-white p-5 shadow-[0_24px_80px_-58px_rgba(15,23,42,0.65)] md:p-6">
          ...
          {displayBooks.length === 0 ? (
            ...
            <div className="mt-8 grid grid-cols-2 gap-x-5 gap-y-8 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">
              {displayBooks.map((b: any) => {
                const actualBook = b.book || b;
                return (
                  <div key={b.book_id || actualBook.book_id || Math.random()} className="relative mx-auto w-full max-w-[180px]">
