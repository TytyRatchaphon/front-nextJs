...
import { useBuyEpisodesMutation, useBuyFullBookMutation, useBuyGroupPromotionMutation } from "@/features/book/hooks/useBookWriteMutations";
...
const BookInfoCard = ({ book, bookId }: BookInfoCardProps) => {
  ...
  const onConfirmPromotionPurchase = async () => {
    if (!book.promotion?.id) return;
    ...
    try {
      ...
      if (user && book.promotion?.price) {
        ...
        if (payWith === "coin") {
          ...
          const expectedFreeCoin = (Number(user.freecoin) || 0) - (Number(book.promotion.price) || 0);
        ...
        {!isDeleted && (
          <>
            <BookInfoCardPurchaseModals
              ...
              promotionConfirmOpen={promotionConfirmOpen}
              onClosePromotionConfirm={() => setPromotionConfirmOpen(false)}
              onConfirmPromotionPurchase={onConfirmPromotionPurchase}
            />
