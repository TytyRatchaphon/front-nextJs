    ...
    <div className="w-full max-w-[1124px] mx-auto px-4 py-8">
      ...
      <div className="flex flex-col md:flex-row justify-between items-center mb-8 border-b-2 border-gray-200 pb-4">
        <div className="flex items-center gap-3 w-full md:w-auto mb-4 md:mb-0">
          ...
          <h1 className="text-2xl md:text-3xl font-bold text-black">จัดอันดับ</h1>
        </div>
        ...
        <div className="flex flex-col sm:flex-row items-center gap-4 w-full md:w-auto">
          ...
          <div className="flex bg-gray-100 rounded-lg p-1 w-full sm:w-auto overflow-x-auto no-scrollbar">
            <button
              onClick={() => handleRangeChange('week')}
              className={`flex-1 sm:flex-none px-3 md:px-4 py-1.5 rounded-md text-[12px] md:text-sm font-medium transition-colors whitespace-nowrap ${range === 'week' ? 'bg-white shadow text-red-600' : 'text-gray-500 hover:text-gray-700'}`}
            >
              สัปดาห์
            </button>
            <button
              onClick={() => handleRangeChange('month')}
              className={`flex-1 sm:flex-none px-3 md:px-4 py-1.5 rounded-md text-[12px] md:text-sm font-medium transition-colors whitespace-nowrap ${range === 'month' ? 'bg-white shadow text-red-600' : 'text-gray-500 hover:text-gray-700'}`}
            >
              เดือน
            </button>
            <button
              onClick={() => handleRangeChange('year')}
              className={`flex-1 sm:flex-none px-3 md:px-4 py-1.5 rounded-md text-[12px] md:text-sm font-medium transition-colors whitespace-nowrap ${range === 'year' ? 'bg-white shadow text-red-600' : 'text-gray-500 hover:text-gray-700'}`}
            >
              ปี
            </button>
            <button
              onClick={() => handleRangeChange('all')}
              className={`flex-1 sm:flex-none px-3 md:px-4 py-1.5 rounded-md text-[12px] md:text-sm font-medium transition-colors whitespace-nowrap ${range === 'all' ? 'bg-white shadow text-red-600' : 'text-gray-500 hover:text-gray-700'}`}
            >
              ตลอดกาล
            </button>
          </div>
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 md:gap-6">
          {books.map((book) => (
            <div key={book.book_id} className="bg-white rounded-xl p-3 md:p-4 flex gap-3 md:gap-6 shadow-sm hover:shadow-md transition-shadow border border-gray-100">
              {/* Ranking Badge & Image */}
              <div className="relative shrink-0 w-[90px] md:w-[140px] aspect-[2/3]">
                <Link href={`/book/${book.book_id}`} className="block w-full h-full relative group overflow-hidden rounded-md">
                  ...
                  <div className="absolute -top-1.5 -left-1.5 z-10">
                    <div className="relative w-7 h-7 md:w-10 md:h-10">
                      ...
                      <div className={`w-full h-full flex items-center justify-center rounded-full text-[10px] md:text-sm font-bold shadow-sm bg-white border ${book.rank === 1 ? 'border-yellow-400 text-yellow-500' :
              ...
              {/* Details */}
              <div className="flex-1 flex flex-col justify-start py-0.5 md:py-1 min-w-0">
                <Link href={`/book/${book.book_id}`}>
                  <h2 className="text-base md:text-xl font-bold text-black mb-0.5 md:mb-1 line-clamp-1 hover:text-red-600 transition-colors leading-tight">{book.name}</h2>
                </Link>
                <div className="text-[11px] md:text-sm text-gray-500 mb-1 md:mb-2 font-medium line-clamp-1">{book.writer_name || 'Unknown Author'}</div>

                <div className="text-[11px] md:text-sm text-gray-400 mb-1.5 md:mb-2 line-clamp-2 md:line-clamp-3 leading-relaxed hidden xs:block">
                  {book.title}
                </div>

                {/* Tags */}
                <div className="flex flex-wrap gap-1 md:gap-2 mb-auto w-full overflow-hidden">
                  <TagSwiper tags={Array.isArray(book.tag) ? book.tag : (typeof book.tag === 'string' ? book.tag.split(',').filter(t => t.trim() !== '') : [])} classImport="px-1.5 md:px-2 py-0.5 rounded-full border border-red-200 text-red-500 text-[9px] md:text-xs bg-red-50/50 whitespace-nowrap" />
                </div>

                {/* Footer Stats */}
                <div className="flex items-center gap-3 md:gap-4 text-[10px] md:text-sm text-gray-400 mt-1.5 md:mt-2">
                  <div className="flex items-center gap-1">
                    ...
                    <span>{Number(book.view).toLocaleString()}</span>
                  </div>
                  <div className="flex items-center gap-1">
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" className="md:w-4 md:h-4">
                    ...
                    <span>{Number(book.chapter).toLocaleString()} ตอน</span>
                  </div>
                </div>
              </div>
            </div>
