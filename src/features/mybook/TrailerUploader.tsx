...
function getPendingStatusText(status: string): string {
  switch (status) {
    ...
    case 'completing': return 'Merging files...';
    case 'queued': return 'Waiting for processing queue...';
    case 'processing': return 'Converting video file...';
...
function getUploadStateText(state: string): string {
  switch (state) {
    case 'initiating': return 'Preparing for upload...';
    case 'uploading': return 'Uploading...์วิดีโอ...';
    case 'completing': return 'Merging files...';
    case 'queued': return 'Waiting for processing queue...';
    case 'processing': return 'Processing video in system...';
    default: return 'Processing...';
  }
}
...
export default function TrailerUploader({
  ...
  useEffect(() => {
    if (mode !== 'select-only' && autoStart && initialFile && uploadState === 'idle' && config && bookId) {
      startUpload(initialFile, {
        onSuccess: (url) => { messageApi.success('Video uploaded successfully!'); onSuccess?.(url); },
  ...
  const handleFileSelect = (file: File): false => {
    if (!config) {
      messageApi.error('System is not ready, please wait a moment');
      return false;
    }

    if (file.size > config.maxUploadBytes) {
      messageApi.error(`File size exceeds limit (${Math.round(config.maxUploadBytes / 1024 / 1024)}MB)`);
      return false;
    }
    ...
    video.onloadedmetadata = () => {
      ...
      if (video.duration > config.maxDurationSeconds) {
        messageApi.error(`Video duration exceeds limit (${config.maxDurationSeconds} seconds)`);
        ...
        <div className="relative z-10">
          {!selectedFile ? (
            ...
            >
              ...
              <p className="ant-upload-text font-medium text-gray-700">Click or drag video file here</p>
              {config && (
                <p className="ant-upload-hint text-gray-400 pb-4">
                  Supported {config.supportedMimeTypes?.map(m => m.split('/')[1]).join(', ') || 'mp4, quicktime, webm'}
                  <br />
                  Max size {Math.round((config.maxUploadBytes || 0) / 1024 / 1024)}MB, Max duration {config.maxDurationSeconds || 0} seconds
                </p>
              )}
            </Upload.Dragger>
          ) : (
  ...
  const handleDelete = () => {
    Modal.confirm({
      ...
      content: 'คุณแน่ใจหรือไม่ว่าต้องการDelete Videoนี้? การดำเนินการนี้ไม่สามารถย้อนกลับได้',
      okText: 'Delete',
      okType: 'danger',
      cancelText: 'Cancel',
      onOk: async () => {
  ...
  const handleCancelUpload = () => {
    Modal.confirm({
      title: 'Cancel Upload',
      icon: <ExclamationCircleOutlined />,
      content: 'คุณแน่ใจหรือไม่ว่าต้องการCancel Upload?',
      ...
      onOk: async () => {
        await cancelUpload();
        setSelectedFile(null);
        messageApi.info('Upload cancelled');
      },
    });
  };
      ...
      <div className="space-y-4 relative z-10">
          ...
          <div className="space-y-4 py-4 px-4 bg-orange-50/50 rounded-xl border border-orange-100">
            <div className="text-center">
              <p className="font-semibold text-gray-800 text-base mb-1">
                {hasCurrent ? '🔄 กำลังChange Videoใหม่...' : getPendingStatusText(videoStatus!.pending!.status)}
              </p>
              <p className="text-blue-600 text-xs font-medium bg-blue-50 inline-block px-3 py-1 rounded-full border border-blue-100">
                ✅ You can close or go to another page. System is working in background
              </p>
            </div>
            <Progress
            ...
            <div className="text-center">
              ...
              {estimatedSeconds !== null && (uploadState === 'uploading' || (uploadState === 'processing' && progress > 0)) && (
                <p className="text-red-500 font-medium text-sm flex items-center justify-center gap-1">
                  ...
                  Estimated time remaining {estimatedSeconds < 60
              ...
              {(uploadState === 'initiating' || uploadState === 'uploading') && (
                <p className="text-orange-500 text-xs mt-2 font-medium bg-orange-50 inline-block px-3 py-1 rounded-full border border-orange-100">
                  ⚠️ Uploading...์ กรุณาอย่าเพิ่งปิดหรือรีเฟรชหน้านี้
                </p>
              )}
              ...
              {(uploadState === 'queued' || uploadState === 'processing' || uploadState === 'completing') && (
                <p className="text-blue-500 text-xs mt-2 font-medium bg-blue-50 inline-block px-3 py-1 rounded-full border border-blue-100">
                  ✅ Upload complete. You can go to another page.
                </p>
              )}
            </div>
        ...
        {uploadState === 'completed' && (
          <div className="text-center space-y-2 py-4">
            ...
            <p className="font-medium text-green-600">Video processing complete!</p>
            {onSuccess && (
              <Button type="primary" onClick={() => onSuccess?.('')}>Continue</Button>
            )}
          </div>
        )}
        ...
        {hasFailed && uploadState !== 'processing' && (
          <div className="py-4 px-4 bg-red-50/50 rounded-xl border border-red-100 space-y-3">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="font-semibold text-red-700 text-sm">Video processing failed</p>
                <p className="text-red-500 text-xs mt-0.5">
                  {videoStatus!.lastFailed!.errorMessage || videoStatus!.lastFailed!.errorCode || 'Unknown error occurred'}
                </p>
              </div>
            </div>
            <div className="flex gap-2">
        ...
        {((!hasCurrent && !hasPending && !hasFailed && !isUploading && uploadState !== 'completed' && uploadState !== 'error' && canUpload)
          ...
          <div className="space-y-4">
            {showReplaceUpload && (
              <div className="flex items-center justify-between">
                <p className="text-sm font-semibold text-gray-700">Select new video</p>
        ...
        {!canUpload && !hasCurrent && !hasPending && !hasFailed && !isUploading && (
          <div className="text-center py-4 text-gray-500 text-sm">
            Cannot upload video at this time
          </div>
        )}
      </div>
    </div>
  ...
  return (
    <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between p-4 bg-white rounded-xl border border-gray-100 shadow-sm hover:border-red-200 transition-colors gap-4">
      ...
      <div className="flex gap-2 w-full sm:w-auto justify-end">
        <Button danger className="rounded-lg font-medium" onClick={onRemove}>Delete</Button>
