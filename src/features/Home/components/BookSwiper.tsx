...
import HomeCollectionCard from '@/features/collection/components/HomeCollectionCard';
...
export default function BookSwiper({ books, title, icon, link, isCollection, onBookClick }: BookSwiperProps) {
      ...
      >
        {books && books.length > 0 ? (
          books.map((item, index) => (
            <SwiperSlide
              key={`${item.book_id ?? item.id ?? 'item'}-${item.parent_book_id ?? 'root'}-${item.content_type ?? 'novel'}-${index}`}
              className={isCollection ? "!w-auto" : "!w-auto"}
            >
              {isCollection ? (
                <HomeCollectionCard collection={item} />
              ) : (
                <CardBook book={item} onBookClick={onBookClick} />
              )}
            </SwiperSlide>
          ))
        ) : (
          // Fallback or empty state
          <div className="text-center py-10 text-gray-400">{isCollection ? "ไม่มีคอลเลคชั่น" : "ไม่มีหนังสือ"}</div>
        )}
      </Swiper>
    </div>
  );
