...
export default function RankingCategoryRight({ categoryId, categoryName, tab }: RankingCategoryRightProps) {
  const [activeTab, setActiveTab] = useState<'weekly' | 'monthly' | 'yearly'>('weekly');

  const { data: books = [], isLoading } = useQuery({
    queryKey: queryKeys.categories.ranking(categoryId, activeTab, tab || 'novel'),
    queryFn: () => {
