  ...
  const { settings } = useWebsiteSettings();
    ...
    const handleDraftSelectableOptionChange = React.useCallback((item: CartItem, option: SelectableOption, checked: boolean) => {
        ...
        if (
            checked &&
            selectionLimit > 1 &&
            !draftSelectedOptionIds.includes(optionId) &&
            draftSelectedOptionIds.length >= selectionLimit
        ) {
        ...
        setDraftSelectedOptionIds((currentSelectedIds) => {
            ...
            if (checked) {
                ...
                if (currentSelectedIds.length >= selectionLimit) return currentSelectedIds;
                return [...currentSelectedIds, optionId];
            }

            return currentSelectedIds.filter((id) => id !== optionId);
        });
    }, [draftSelectedOptionIds, getSelectionLimit, notification]);
