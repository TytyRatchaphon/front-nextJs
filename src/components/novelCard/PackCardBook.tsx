            ...
            <div
                ...
                onClick={() => {
                    if (onClick) {
                        ...
                        // Fallback or do nothing if no onClick provided
                        // postBookClick is handled by parent if needed? no, parent handles modal
                    }
                }}
            >
                <div className="flex flex-col w-full h-full rounded-lg bg-white shadow-sm hover:shadow-md transition-shadow group/card cursor-pointer relative overflow-visible">
                    {/* Image Container */}
                    <div className="relative w-full aspect-[2/3] md:h-[237px]">
                        {!imgError ? (
                            <Image
                                src={imageUrl}
                                alt={book.name ?? ''}
                    ...
                    {(() => {
                        ...
                        const badges = {
                            ...
                            discount: (percent: number | string) => ({
                                component: (
                                    <div className="absolute -top-3 -right-3 z-20 w-[60px] h-[60px] md:w-[75px] md:h-[75px]">
                                        <Image
                                            ...
                                            loader={imageLoader}
                                        />
                                        <span className="absolute top-[50px] md:top-[64px] right-[14px] md:right-[18px] text-white text-[12px] md:text-[15px] leading-none transform -rotate-[17deg]">{percent}%</span>
                                    </div>
                                )
                            }),
                            ...
                            ended: (pos: 'left' | 'right') => ({
                                component: (
                                    <div className={`absolute top-2 ${pos === 'left' ? 'left-2' : 'right-2'} bg-gradient-to-r from-emerald-400 to-teal-500 text-white px-2 py-0.5 md:px-3 md:py-1 rounded-full text-[10px] md:text-xs font-medium shadow-md z-20`}>
                                        จบแล้ว
                                    </div>
                                )
                            })
                    ...
                    <div className="px-3 pt-3 pb-2 flex flex-col gap-1 flex-1">
                        ...
                        <div className="mt-auto grid grid-cols-3 items-center text-[10px] md:text-xs text-gray-500 w-full">
