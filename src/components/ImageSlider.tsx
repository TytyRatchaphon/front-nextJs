...
export function NewNovelSlider() {
    const { data:novels, isLoading, error } = useGetNodeNovel();
        ...
        <SwiperSlide>
            <div className='swiper-slide items-start SwiperSlide' style={{width: "151.667px", marginRight: "10px"}}>
                <Link className='flex flex-col cursor-pointer p-2 text-start  hover:text-primary bg-transparent' href={`/book/${novel.book_id}`} style={{width: "100%", height: "auto"}}>
                   ...
                   <div className=''>
                    <span className=' font-semibold line-clamp-2 leading-[1.2] min-h-[2.4rem] overflow-x-hidden mt-1 hover:text-primary text-black'>{novel.title}</span>
                    <span className='text-black text-sm text-nowrap line-clamp-1 overflow-x-hidden'>{novel.by}</span>
                   </div>
