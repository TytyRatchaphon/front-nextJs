...
function CardBook({ book }: CardBookProps) {
  ...
  React.useEffect(() => {
    try {
      console.debug('CardBook debug:', {
        ...
        bookID: book.bookID,
        shelveCount: book.shelveCount || book.shelfCount || book.shelve_count || book.shelf_count,
        writer: book['writer.writer_name'] || book.writer_name || book.author,
      });
    } catch (e) {
      // ignore
    }
  ...
  return (
  <div className="w-full max-w-[168px] h-[355px] flex-shrink-0 relative z-0">
    <Link 
      href={`/book/${encodeURIComponent(bookParam)}`} 
      className="flex flex-col cursor-pointer p-2 text-start hover:text-red-600 bg-transparent relative overflow-visible"
    ...
    >
      <div className="flex flex-col w-full h-full rounded-lg bg-white shadow-sm hover:shadow-md transition-shadow group/card cursor-pointer relative overflow-visible">
        ...
        {/* Status Badges */}
        {(() => {
            // Config for each badge type: image path, width, height, and custom styles (top/right/etc)
            let badgeConfig = null;

            // Priority: BestSeller > New > Discount > NewEp
            if (book.isBestSeller) {
              badgeConfig = { 
                src: "/images/bestseller.png", 
                width: 46, 
                height: 54, 
                className: "absolute -top-2 -right-0 z-20" // ปรับตำแหน่ง BestSeller ให้เลยขึ้นไปข้างบนและขวา
              };
            } else if (book.isNew) {
              badgeConfig = { 
                src: "/images/new.png", 
                width: 50, 
                height: 50, 
                className: "absolute top-3 right-24 z-20" // ปรับตำแหน่ง New ตรงนี้
              };
              ...
              badgeConfig = { 
                ...
                width: 36, 
                height: 36, 
                className: "absolute top-0 right-2 z-20" // ปรับตำแหน่ง Promotion ตรงนี้
              };
            } else if (book.isNewEp) {
              badgeConfig = { 
                src: "/images/new.png", 
                width: 36, 
                height: 36, 
                className: "absolute top-0 right-0 z-20" // ปรับตำแหน่ง NewEp ตรงนี้
              };
            }

            if (ended && badgeConfig) {
              return (
                <>
                  <div className="absolute top-2 left-2 bg-gradient-to-r from-emerald-400 to-teal-500 text-white px-3 py-1 rounded-full text-xs font-medium shadow-md z-20">
                    จบแล้ว
                  </div>
                  <div className={`${badgeConfig.className} z-20`}>
                    <Image 
                      src={badgeConfig.src} 
                      alt="badge" 
            ...
            if (ended) {
              return (
                <div className="absolute top-2 right-2 bg-gradient-to-r from-emerald-400 to-teal-500 text-white px-3 py-1 rounded-full text-xs font-medium shadow-md z-20">
                  จบแล้ว
                </div>
              );
            }
  ...
  <div className="px-3 pt-3 pb-2 flex flex-col gap-1 flex-1">
          <h3 className="text-black text-md font-primary font-medium group-hover/card:text-red-600 transition-colors duration-300 line-clamp-1 min-h-[2.5rem]">
            {book.name}
          </h3>
          ...
          <div className="mt-auto grid grid-cols-3 items-center text-xs text-gray-500 w-full">
            ...
            <div className="flex items-center gap-1 justify-end">
              ...
              <span className="leading-none">{formatNumber(book.chapter || 0)}</span>
            </div>
          </div>
        </div>
      </div>
