...
const handleConfirmStampExchange = async () => {
    ...
    try {
        ...
        setTimeout(async () => {
            try {
                ...
                if (profile) {
                    ...
                    console.debug('AllEvent: final sync complete (stamp)', profile)
                }
            } catch (e) {
                console.warn('AllEvent: failed to fetch /user/me after stamp-exchange', e)
            }
        }, 1000) 
      }
      ...
      console.error('Stamp exchange failed', e)
        ...
        setTimeout(async () => {
            try {
                ...
                if (profile) {
                    ...
                    localStorage.setItem('userData', JSON.stringify(profile))
                    console.debug('AllEvent: final sync complete', profile)
                }
            } catch (e) {
                console.warn('AllEvent: failed to fetch /user/me', e)
            }
        }, 1000) 
      }

    } catch (e) {
      console.error('Gacha API failed', e)
      ...
      >
        {stage === 1 && (
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 16, width: '100%' }}>
            <div style={{ flex: '0 0 auto' }}>
              <Image src="/images/gachaStatic.gif" alt="gacha" width={220} height={220} style={{ objectFit: 'contain' }} unoptimized />
            </div>
        ...
        {stage === 3 && (
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 12, width: '100%' }}>
            {/* Show result from API if present */}
              ...
              {gachaResult ? (
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8 }}>
                  {(() => {
                    ...
                    return (
                      <>
                        <Image src={img} alt={label} width={96} height={96} style={{ objectFit: 'contain' }} />
