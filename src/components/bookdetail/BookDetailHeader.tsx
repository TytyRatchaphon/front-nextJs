...
const BookDetailHeaderContent = ({ book }: BookDetailHeaderProps) => {
  ...
  const [isFollowed, setIsFollowed] = useState(book.isFollowingWriter || false);
  ...
  useEffect(() => {
    setIsFollowed(book.isFollowingWriter || false);
  }, [book.isFollowingWriter]);
        ...
        <div className="relative flex flex-col lg:flex-row gap-4 sm:gap-6 items-start p-4 sm:p-6">
          ...
          <div className="flex-1 w-full lg:w-auto">
            ...
            <div className="flex items-center gap-2 mb-2 sm:mb-3 flex-wrap">
              ...
              {currentUserId && book.writerId && String(currentUserId) === String(book.writerId) ? (
            ...
            <div className="flex gap-2 sm:gap-3 mt-3 sm:mt-4 flex-wrap">
              <Link
                href={book.firstEpisodeId ? `/read/${book.id}/${book.firstEpisodeId}` : '#'}
                className={`!bg-red-600 !text-white px-3 sm:px-4 lg:px-5 py-2 sm:py-2.5 rounded-lg text-xs sm:text-sm hover:!bg-red-700 transition flex items-center gap-1.5 sm:gap-2 shadow-sm ${!book.firstEpisodeId ? 'opacity-50 cursor-not-allowed' : ''}`}
                onClick={(e) => {
                  if (!book.firstEpisodeId) e.preventDefault();
                }}
              >
              ...
              >
                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
                  <path d="M9 22H15C20 22 22 20 22 15V9C22 4 20 2 15 2H9C4 2 2 4 2 9V15C2 20 4 22 9 22Z" fill={isAdded ? "#ffecec" : "white"} stroke="#B01F1F" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
