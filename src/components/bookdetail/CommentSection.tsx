...
export default function CommentSection({ bookId, mode = "comment" }: CommentSectionProps) {
  ...
  const handleSubmit = async () => {
    ...
    try {
        ...
        const response = await postBookReview(bookId, commentToSend, rating);
    ...
    <div className="px-2 sm:px-4 lg:px-6 pb-8">
      ...
      <div className="bg-white rounded-xl border border-gray-100 p-6 shadow-sm mb-8">
        ...
        <div className="space-y-4">
            <div>
              <div className="mb-2 text-sm font-medium text-gray-700">ให้คะแนนเรื่องนี้</div>
              <Rate value={rating} onChange={setRating} className="text-red-500 text-2xl" />
            </div>
