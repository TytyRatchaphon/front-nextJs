...
export default function BuyMultipleEpisodesModal({
  ...
  const totalCost = useMemo(() => {
    return selectedEpisodes.reduce((sum, epId) => {
      const episode = purchasableEpisodes.find((ep) => ep.epID === epId);
  ...
  const handleSelectAll = () => {
    if (selectedEpisodes.length === purchasableEpisodes.length) {
      ...
      setSelectedEpisodes(purchasableEpisodes.map((ep) => ep.epID));
    }
  };
              ...
              purchasableEpisodes.map((episode) => (
                <div
                  ...
                  className={`flex items-center justify-between p-3 border-b last:border-b-0 hover:bg-gray-50 cursor-pointer ${
                    selectedEpisodes.includes(episode.epID) ? "bg-blue-50" : ""
                  }`}
                  onClick={() => handleToggleEpisode(episode.epID)}
                >
                  <div className="flex items-center gap-3 flex-1">
                    <Checkbox
                      checked={selectedEpisodes.includes(episode.epID)}
                      onChange={() => handleToggleEpisode(episode.epID)}
                    />
