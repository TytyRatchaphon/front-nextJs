...
import { parseJwt } from "@/utils/socialLogin";
import {
  GOOGLE_CLIENT_ID,
  loadGoogleScript,
  initializeGoogleSignIn,
  handleGoogleLogin as handleGoogleLoginUtil,
  triggerGoogleSignIn,
} from "@/utils/login-google";
import {
  LIFF_ID,
  loadLIFFScript,
  initializeLIFF,
  handleLineLogin as handleLineLoginUtil,
} from "@/utils/login-line";

// Declare global types for Google Sign-In และ LIFF
declare global {
  interface Window {
    google?: any;
    liff?: any;
...
const LoginButtonHeader: React.FC = () => {
  ...
  // -------------------- Load Scripts และ Initialize --------------------
  useEffect(() => {
    const loadScripts = async () => {
      try {
        // โหลด Google และ LINE scripts พร้อมกัน
        await Promise.all([loadGoogleScript(), loadLIFFScript()]);

        // Initialize LIFF
        await initializeLIFF(LIFF_ID);

        setScriptsLoaded(true);
        console.log("All social login scripts loaded successfully");
      } catch (error) {
        console.error("Error loading social login scripts:", error);
      }
    };
  ...
  const handleGoogleLogin = async (response: any) => {
    await handleGoogleLoginUtil(response, login, handleCancel);
  };

  // -------------------- LINE Login Handler --------------------
  const handleLineLogin = async () => {
    await handleLineLoginUtil(login, handleCancel);
  };

  // -------------------- Initialize Google Sign-In when modal opens --------------------
  useEffect(() => {
    if (isLoginModalOpen && scriptsLoaded && window.google) {
      // Set global callback function
      window.handleCredentialResponse = handleGoogleLogin;

      // Initialize Google Sign-In
      initializeGoogleSignIn(GOOGLE_CLIENT_ID, handleGoogleLogin);

      console.log("Google Sign-In initialized");
    }
  }, [isLoginModalOpen, scriptsLoaded]);
  ...
  const onRegisterFinish: FormProps<RegisterFieldType>["onFinish"] = (
    values
  ) => {
          ...
          {loginViewMode === "login" ? (
              ...
              <div className="text-center bg-white rounded-r-xl p-6 flex flex-col justify-between">
                <div>
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    ...
                    <div
                      id="g_id_onload"
                      data-client_id="36558994619-rr5hree4o8jgjiugvrjprtj3padreg6o.apps.googleusercontent.com"
                      data-callback="handleCredentialResponse"
                      data-auto_prompt="false"
                      style={{ display: "none" }}
                    ></div>
                    <div
                      className="border border-gray-200 rounded-md py-2 flex justify-center items-center cursor-pointer hover:bg-blue-50"
                    ...
                    <div className="border border-gray-200 rounded-md py-2 flex justify-center items-center cursor-pointer hover:bg-blue-50">
                      <Image
                        src="/_next/image?url=%2F_next%2Fstatic%2Fmedia%2Ffacebook.63de5bea.png&w=256&q=75"
                        alt="Facebook"
                        className="inline-block h-[23px] w-[23px] rounded-full "
                        loading="lazy"
                        width="96"
                    ...
                    {/* LINE Login */}
                    <div
                      className="border border-gray-200 rounded-md py-2 flex justify-center items-center cursor-pointer hover:bg-green-50"
                  ...
                  <div className="flex items-center my-2">
                    ...
                    <div className="mx-4 text-primary text-sm font-medium font-primary text-red-600">
                      หรือ
                    </div>
                    <div className="flex-grow border-b border-primary text-red-600"></div>
                    <div className="flex-grow"></div>
                  ...
                  >
                    <Form.Item<LoginFieldType>
                      label={
                        <span className="text-sm text-black md:text-base text-left block w-full font-medium font-primary">
                          อีเมล
                        </span>
                      }
                      name="email"
                    ...
                    <div className="grid grid-cols-2 p-0 ">
                      <div className="flex justify-start items-center">
                        <a
                          href=""
                          className="text-sm underline text-primary cursor-pointer md:text-xl font-medium font-primary text-red-600 login-link"
                        >
                          <span className="font-primary text-red-600 underline">
                            ลืมรหัสผ่าน
                          </span>
                        </a>
                      </div>
                      <div className="flex justify-end p-0">
                        <Button
                          ...
                          danger
                          size="large"
                          className="font-medium font-primary text-md text-red-500 py-1 hover:border-secondary hover:bg-secondary hover:text-primary focus:outline-none fontFam md:text-xl"
                        >
                ...
                <div>
                  <div className="grid grid-cols-2 p-0 gap-3 mt-5 ">
                    <div className="flex justify-end items-center">
                      <span className="text-[12px] text-gray-600 md:text-lg font-medium font-primary">
                        ยังไม่มีบัญชีผู้ใช้?
                      </span>
                    </div>
                    <div className="flex justify-start p-0">
                      <a
                        ...
                        className="text-sm text-primary cursor-pointer underline font-bold md:text-lg font-primary login-link"
                      >
                        <span className="font-primary text-red-600 underline">
                          สมัครสมาชิก
                        </span>
                  ...
                  <a
                    target="_blank"
                    className="text-sm cursor-pointer font-bold mt-5 inline-block font-primary login-link"
                    href="/policy-privacy"
                  >
                    <span className="font-primary text-red-600 underline">
            ...
            <div className="grid lg:grid-cols-2 p-0 gap-0">
              ...
              <div className="text-center rounded-l-xl bg-white p-6 flex flex-col justify-between">
                <div>
                  <div className="mb-2">
                    <span className="text-primary font-bold md:text-xl text-red-600 font-primary">
                      สมัครสมาชิก
                    </span>
                  </div>
                  <Form
                  ...
                  >
                    <Form.Item<RegisterFieldType>
                      label={
                        <span className="text-sm text-black md:text-base text-left block w-full font-primary">
                          ชื่อ-นามสกุล
                        </span>
                      }
                      name="fullname"
                    ...
                    <div className="flex flex-col text-center py-4">
                      <span className="text-sm text-gray-500 font-primary">
                        กดปุ่ม "สมัครสมาชิก" เป็นการยอมรับ
                      </span>
                      <a
                        href="#"
                        className="text-sm text-primary font-bold cursor-pointer font-primary "
                      >
                    ...
                    <div className="grid grid-cols-2 items-center">
                      <div className="flex justify-start">
                        ...
                        <a
                          ...
                          className="text-sm underline text-primary cursor-pointer font-primary login-link"
                        >
